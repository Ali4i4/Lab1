...
int window (int vvod, int count)                                    //функция построения этажа с окнами
{
    ...
    d = ((vvod - 2)/2 - 1)/3;                                       // треть высоты этажа
    int s = (vvod - 2)/12 + 1;                                      // для счёта количества окон (на каждые 12 единиц ширины одно окно)
    for (c = 1; c <= count - 1; c++) 
    {
        for (h = 1; h <= d - 1; h++)                                // пустые строки после крыши перед окном
        {
            cout << a;
            for (n = 1; n <= vvod - 2; n++)                         // ширина - 2 = ширина дома без клеток для левой и правой стенок дома
                cout << ' ';
            cout << b; 
        }
        cout << a;                                                  // описание строки с верхней рамой окна
        for (n = 1; n <= (vvod - 2)/4/s; n++)                       // четверть ширины этажа = расстояние между стеной и окном
            cout << ' ';
        n = 1;
        while (n <= s)                                              // описание окон
        {
            cout << ' ';
            for (i = 1; i <= ((vvod - 2) - ((vvod - 2)/4/s - (vvod - 2)/4/s) - 2*s)/s; i++) // верхняя рама одного окна = (ширина этажа без 2х клеточек для боковых стен) - (расстояние между стеной и окном с двух сторон) - 2 * количество оконных рам на этаже )
                cout << '_';                                        // верхняя рамка
            cout << ' ';
            n++;
        }
        for (n = 1; n <= (vvod - 2)/4/s + ((vvod - 2) - (vvod - 2)/4/s - (vvod - 2)/4/s - 2*s) % s; n++) // оставшееся расстояние между окном и правой стеной = четверть ширины + остаток невошедшего расстояния при выполнении прошедших шагов ( возникает при нечётных значениях) 
            cout << ' ';
        cout << b;
        for (h = 1; h <= ((vvod - 2)/2 - 1) - 1 - 2*d; h++)             // база окна
        {
            cout << a;
